...
export default function PeopleChoirPage() {
  ...
  const handleAddMember = (e: React.FormEvent) => {
    ...
    showToast(t(`Vocalist ${nameEn} registered!`, `ዘማሪ ${nameAm || nameEn} ተመዝግቧል!`));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">
            {t('Choir & Hymnody Members', 'የዝማሬ እና ዜማ አባላት')}
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            {t(
              'Parish Sunday school singers, cantors, instrumentalists, and liturgical vocalists',
              'የሰንበት ት/ቤት ዘማሪዎች፣ አዝማሪዎችና የመሳሪያ ተጫዋቾች'
            )}
          </p>
        </div>
        <button
        ...
        >
          <UserPlus size={16} />
          {t('Register Vocalist', 'አዲስ ዘማሪ መዝግብ')}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="card p-5">
          ...
          <div className="text-xs text-slate-500 mt-1">{t('Total Vocalists', 'አጠቃላይ ዘማሪዎች')}</div>
        </div>
        <div className="card p-5">
          <div className="text-2xl font-bold text-emerald-600">
            {members.filter((c) => c.status === 'ACTIVE').length}
          </div>
          <div className="text-xs text-slate-500 mt-1">{t('Active in Choir', 'ንቁ ዘማሪዎች')}</div>
        </div>
        <div className="card p-5">
          <div className="text-2xl font-bold text-purple-600">4</div>
          <div className="text-xs text-slate-500 mt-1">{t('Vocal Ranges', 'የድምፅ ደረጃዎች')}</div>
        </div>
        <div className="card p-5">
          <div className="text-2xl font-bold text-blue-600">5</div>
          <div className="text-xs text-slate-500 mt-1">{t('Assigned Vestments', 'የተመደቡ አልባሳት')}</div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        <div className="table-container rounded-none border-0">
          <table>
            <thead>
              <tr>
                <th>{t('Member Name', 'የዘማሪ ስም')}</th>
                <th>{t('Baptismal Name', 'የክርስትና ስም')}</th>
                <th>{t('Voice Range', 'የድምፅ ደረጃ')}</th>
                <th>{t('Instrument', 'የሙዚቃ መሳሪያ')}</th>
                <th>{t('Hymn Disciplines', 'የዜማ ዘርፎች')}</th>
